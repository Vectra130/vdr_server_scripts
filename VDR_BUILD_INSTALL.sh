...
for folder in "root/.vdr" usr/bin usr/lib/vdr/plugins-$APIVERSION usr/share/locale/de_DE/LC_MESSAGES; do
...
copy(){
	for ii in $(find PLUGINS/src/ -maxdepth 2 -type d | grep src/.*/$i); do
		if [ "$(basename $ii)" == "conf" ]; then
			iii=$(echo "$(dirname $ii)" | sed -e 's!PLUGINS/src/!!')
		else
...
for ii in $(find PLUGINS/src/ -type f -iname *.theme); do
#for ii in $(find PLUGINS/src/ -maxdepth 2 -type d | grep src/.*/$i); do
	cp $ii $THISDIR/
done
...
for i in icons logos symbols decors fonts conf services dtd views skins fonts scripts; do
	copy
done
echo -e " \e[32mOK\e[0m"
}
